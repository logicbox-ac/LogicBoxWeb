...
var ExtensionBlocks = /*#__PURE__*/function () {
  ...
  function ExtensionBlocks(runtime) {
    ...
    /**
     * Full-device refresh in flight after permission or device changes.
     * @type {Promise<InputDeviceInfo[]> | null}
     */
    this._deviceProbePromise = null;
  ...
  _createClass(ExtensionBlocks, [{
    ...
    value: function getVideoDevicesMenu() {
      if (this._hasIncompleteVideoDeviceList()) {
        this._probeVideoDevices();
      }
      var defaultValues = [{
      ...
      //   }
      // }
      var unnamedDeviceLabel = wrapZWSP(formatMessage({
        id: 'cameraselector.deviceName',
        default: translations.en['cameraselector.deviceName'],
        description: 'fallback label for camera devices without a public name'
      }));
      var deviceValues = this._videoDevices.map(function (dev) {
        var label = dev.label || unnamedDeviceLabel;
        var value = label.match(/[0-9a-f:\.-]{8}/i) ? label : label + "\u200B [" + dev.deviceId.substring(0, 8) + ']';
        return {
          text: value,
          value: value
        };
    ...
    /**
     * Returns true when the visible device list is probably still missing cameras.
     * Browsers often need camera permission before exposing the full list.
     * @private
     * @returns {boolean}
     */
  }, {
    key: "_hasIncompleteVideoDeviceList",
    value: function _hasIncompleteVideoDeviceList() {
      if (!this._videoDevices.length) {
        return true;
      }
      if (this._videoDevices.length === 1) {
        var dev = this._videoDevices[0];
        return !dev.label || !dev.deviceId || dev.deviceId === 'default' || dev.deviceId === 'communications';
      }
      return this._videoDevices.some(function (dev) {
        return !dev.label;
      });
    }

    /**
     * Updates the cached list of visible camera devices and refreshes menus.
     * @private
     * @returns {Promise<InputDeviceInfo[]>}
     */
  }, {
    key: "_enumerateVideoDevices",
    value: function _enumerateVideoDevices() {
      var _this2 = this;
      return navigator.mediaDevices.enumerateDevices().catch(function () {
        ...
        if (_this2.runtime && typeof _this2.runtime.requestBlocksUpdate === 'function') {
          _this2.runtime.requestBlocksUpdate();
        }
        return _this2._videoDevices;
      });
    }

    /**
     * Forces a permission-backed refresh so browsers expose all available cameras.
     * @private
     * @returns {Promise<InputDeviceInfo[]>}
     */
  }, {
    key: "_probeVideoDevices",
    value: function _probeVideoDevices() {
      var _this3 = this;
      if (!navigator.mediaDevices || typeof navigator.mediaDevices.getUserMedia !== 'function') {
        return Promise.resolve(this._videoDevices);
      }
      if (this._deviceProbePromise) {
        return this._deviceProbePromise;
      }
      var finalizeProbe = function finalizeProbe(result) {
        _this3._deviceProbePromise = null;
        return result;
      };
      if (this._tracks.length) {
        this._deviceProbePromise = this._enumerateVideoDevices().then(finalizeProbe, function () {
          return finalizeProbe(_this3._videoDevices);
        });
        return this._deviceProbePromise;
      }
      this._deviceProbePromise = getUserMedia({
        audio: false,
        video: {
          width: {
            ideal: 640
        ...
        return _this3._enumerateVideoDevices().then(function (devices) {
          stream.getTracks().forEach(function (track) {
            return track.stop();
          });
          return finalizeProbe(devices);
        }, function () {
          stream.getTracks().forEach(function (track) {
            return track.stop();
          });
    ...
    value: function _registerListeners() {
      ...
      if (navigator.permissions && typeof navigator.permissions.query === 'function') {
        navigator.permissions.query({
          ...
          return p.addEventListener('change', function () {
            return _this4._onChangeMediaDevice();
          });
        }).catch(function () {});
      }
      ...
      this._videoChangeWatchdogTimeout = setInterval(function () {
        return _this4._videoChangeWatchdog();
      }, 200);
    }
    ...
    value: function _onChangeMediaDevice() {
      var _this5 = this;
      this._enumerateVideoDevices().then(function () {
        if (_this5._hasIncompleteVideoDeviceList()) {
          return _this5._probeVideoDevices();
        }
      }).then(function () {
        // 指定デバイスが見つかったならカメラを切り替える
        var dev = _this5._findVideoDevice(_this5._desiredVideoTrackConstraints);
        if (dev != null && dev.deviceId) {
          _this5._desiredVideoTrackConstraints = {
            deviceId: dev.deviceId
          };
          _this5._openVideoStream(_this5._video);
        }
      });
    }
